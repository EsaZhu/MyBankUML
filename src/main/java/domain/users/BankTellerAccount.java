...
public class BankTellerAccount implements IUser {
    ...
    // Public helpers callable by frontend/API (no interactive prompts)
    public boolean depositForCustomer(String userID, double amount) {
    ...
    public boolean withdrawForCustomer(String userID, double amount) {
        UserAccount acc = getUserByID(userID);
        if (acc == null) return false;
        Account account = getFirstAccount(acc);
        if (account == null) return false;
    ...
    public boolean transferForCustomer(String srcUserID, String dstUserID, double amount) {
        ...
        if (srcAcc == null || dstAcc == null) return false;
